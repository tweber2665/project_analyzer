...
def print_header():
    ...
    print(
        f"{Colors.BOLD}{Colors.CYAN}         PROJECT ANALYZER - Interactive Tool{Colors.END}"
    )
    ...
    while True:
        ...
        if project_name:
            ...
            if sanitized != project_name:
                print(
                    f"{Colors.YELLOW}Note: Project name sanitized to: {sanitized}{Colors.END}"
                )
            return sanitized
        print(
            f"{Colors.RED}Project name cannot be empty. Please try again.{Colors.END}"
        )
...
def get_project_path():
    ...
    while True:
        project_path = input(
            f"{Colors.GREEN}Enter project path (or press Enter for current directory): {Colors.END}"
        ).strip()
        if not project_path:
            project_path = "."
        ...
        if path.exists() and path.is_dir():
            ...
            print(
                f"{Colors.RED}Invalid path. Please enter a valid directory path.{Colors.END}"
            )
...
def select_analyzers():
    ...
    print(
        f"(Press {Colors.BOLD}Enter{Colors.END} to accept defaults, or type Y/N for each)\n"
    )
    ...
    for i, analyzer in enumerate(analyzers):
        ...
        response = (
            input(f"   Run this analyzer? [{Colors.GREEN}Y{Colors.END}/n]: ")
            .strip()
            .lower()
        )
    ...
    if not selected:
        print(
            f"{Colors.YELLOW}No analyzers selected. Selecting all by default.{Colors.END}"
        )
        selected = analyzers
...
def run_analyzer(analyzer, project_path, project_name):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(
                f"{Colors.GREEN}✓ {analyzer['name']} completed in {elapsed_time:.2f} seconds{Colors.END}"
            )
...
def print_summary(project_name, selected_analyzers, results):
    ...
    if successful == total:
        ...
        print(
            f"{Colors.YELLOW}⚠ {successful}/{total} analyzers completed successfully.{Colors.END}"
        )
...
def confirm_execution(project_name, project_path, selected_analyzers):
    ...
    response = (
        input(f"\n{Colors.GREEN}Proceed with analysis? [Y/n]: {Colors.END}")
        .strip()
        .lower()
    )
...
def main():
    ...
    try:
        ...
        print(f"\n{Colors.BOLD}{Colors.BLUE}{'='*60}{Colors.END}")
        print(
            f"{Colors.BOLD}{Colors.GREEN}Analysis complete! Thank you for using Project Analyzer.{Colors.END}"
        )
        print(f"{Colors.BOLD}{Colors.BLUE}{'='*60}{Colors.END}\n")
