...
class ProjectPathsAnalyzer:
    def __init__(self, root_dir: str):
        ...
        self.output_dir = Path(
            f"/users/timweber/dev/{self.project_name}/project_analysis/paths"
        )
    ...
    def analyze_project(self):
        ...
        print(
            f"Total paths found: {sum(len(paths) for paths in self.paths_by_category.values())}"
        )
        ...
        if self.paths_by_category:
            ...
            for category, paths in sorted(
                self.paths_by_category.items(), key=lambda x: len(x[1]), reverse=True
            ):
                unique_paths = len(set(p["path"] for p in paths))
                security = self.path_analyzer.categorize_security_risk(category)
                print(
                    f"  - {category.replace('_', ' ').title()}: {len(paths)} occurrences ({unique_paths} unique) [{security} risk]"
                )
        ...
        report = {
            "metadata": {
                ...
                "total_paths_found": sum(
                    len(paths) for paths in self.paths_by_category.values()
                ),
            },
        ...
        with open(security_csv, "w", newline="", encoding="utf-8") as f:
                    ...
                    security_groups[security_level].append(
                        {
                            "category": category,
                            "path": path,
                            "recommendation": self.path_analyzer.get_best_practice(
                                category
                            ),
                        }
                    )
            ...
            for level in ["critical", "high", "medium", "low"]:
                if level in security_groups:
                    for item in sorted(
                        security_groups[level], key=lambda x: x["path"]["file"]
                    ):
                        writer.writerow(
                            [
                                level.upper(),
...
"""
        ...
        for level in ["critical", "high"]:
            level_categories = [
                (cat, paths)
                for cat, paths in self.paths_by_category.items()
...
"""
                for category, paths in sorted(
                    level_categories, key=lambda x: len(x[1]), reverse=True
                ):
                    unique_paths = list(set(p["path"] for p in paths))
                    category_info = self.path_analyzer.PATH_CATEGORIES.get(category, {})
...
"""
                    ...
                    if len(unique_paths) > 10:
                        html_content += (
                            f"<li>... and {len(unique_paths) - 10} more</li>"
                        )
                    html_content += """
                </ul>
            </details>
