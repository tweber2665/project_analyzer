...
class VariableAnalyzer:
    ...
    def should_analyze_file(self, file_path: Path) -> bool:
        ...
        if any(
            ...
            for pattern in ["sample", "example", "test_data", "mock_data"]
        ):
    ...
    def categorize_variable(self, var_name: str, context: str, file_path: str) -> str:
        ...
        for category, config in self.USAGE_CATEGORIES.items():
            ...
            if "context_patterns" in config:
                if any(
                    pattern in context_lower for pattern in config["context_patterns"]
                ):
                    return category
    ...
    def generate_description(self, var_name: str, context: str, category: str) -> str:
        ...
        if category in self.USAGE_CATEGORIES:
            ...
            if "url" in var_lower:
                ...
                return base_description.split(" (")[
                    0
                ]  # Remove the pattern name in parentheses
...
class ProjectAnalyzer:
    def __init__(self, root_dir: str):
        ...
        self.output_dir = Path(
            f"/users/timweber/dev/{self.project_name}/project_analysis/variables"
        )
        self.files = []
    ...
    def scan_files(self):
        ...
        print(
            "Scanning project files (excluding docs, notebooks, binaries, data files)..."
        )
        total_files = 0
        skipped_files = 0
        ...
        for root, dirs, files in os.walk(self.root_dir):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in self.variable_analyzer.EXCLUDED_DIRS
        ...
        print(
            f"Found {len(self.files)} analyzable source files out of {total_files} total files"
        )
    ...
    def analyze_python_file(self, file_path: str):
        ...
        try:
            ...
            if (
                path_obj.name in ["setup.py", "__init__.py"]
                and path_obj.stat().st_size < 100
            ):
    ...
    def analyze_generic_file(self, file_path: str):
        ...
        try:
            ...
            if file_path.endswith(".env") or file_name == ".env":
                ...
                var_pattern = (
                    r"(?:let|const|static)\s+(?:mut\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s*="
                )
            elif file_ext == ".rb":
            ...
            for i, line in enumerate(lines):
                ...
                if match:
                    ...
                    if file_ext in [".json", ".yml", ".yaml"] and var_name.lower() in [
                        "id",
                        "name",
                        "type",
                        "value",
    ...
    def _extract_paths_from_content(self, content: str, file_path: str):
        ...
        for match in re.finditer(url_pattern, content):
            url = match.group(1)
            line_num = content[: match.start()].count("\n") + 1
            self.paths["url"][url].append({"file": file_path, "line": line_num})

    def analyze_project(self):
        """Analyze all files in the project"""
        print(
            "Analyzing project files with best practice-based variable categorization..."
        )
        ...
        print(
            f"Found {len(self.variables_detailed)} variable instances across {len(vars_by_category)} categories:"
        )

        for category, count in sorted(
            vars_by_category.items(), key=lambda x: x[1], reverse=True
        ):
    ...
    def generate_csv_report(self, prefix: str = "project_analysis"):
        ...
        with open(summary_csv, "w", newline="", encoding="utf-8") as f:
            ...
            for category, var_names in sorted(vars_by_category.items()):
                ...
                # Get category info
                category_info = self.variable_analyzer.USAGE_CATEGORIES.get(
                    category, {}
                )
                description = category_info.get(
                    "description", "General purpose variables"
                )
