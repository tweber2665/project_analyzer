...
class FunctionAnalyzer:
    ...
    def categorize_function(
        self, func_name: str, context: Dict[str, Any], file_path: str
    ) -> str:
        ...
        for category, config in self.FUNCTION_CATEGORIES.items():
            if "file_patterns" in config:
                if any(
                    pattern in file_path_lower for pattern in config["file_patterns"]
                ):
        ...
        for category, config in self.FUNCTION_CATEGORIES.items():
            if "context_patterns" in config:
                if any(
                    pattern in body_content for pattern in config["context_patterns"]
                ):
                    return category
    ...
    def get_function_description(
        self, func_name: str, category: str, context: Dict[str, Any]
    ) -> str:
...
class ProjectFunctionsAnalyzer:
    def __init__(self, root_dir: str):
        ...
        self.output_dir = Path(
            f"/users/timweber/dev/{self.project_name}/project_analysis/functions"
        )
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) or isinstance(
                    ...
                    self.functions_detailed[func_key] = {
                        ...
                        "docstring": (
                            context["docstring"][:200] if context["docstring"] else None
                        ),
                    }
                ...
                for pattern in patterns:
                    ...
                    if match:
                        ...
                        context = {
                            ...
                            "is_generator": "function*" in line
                            or "yield" in " ".join(lines[i : i + 10]),
                            "is_method": not line.strip().startswith("function")
                            and not "=" in line,
                            "class_name": None,
                            "parameters": [],
                            "docstring": None,
    ...
    def analyze_project(self):
        ...
        for root, dirs, files in os.walk(self.root_dir):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in self.function_analyzer.EXCLUDED_DIRS
        ...
        if funcs_by_category:
            ...
            for category, count in sorted(
                funcs_by_category.items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def calculate_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "total_methods": sum(
                1 for f in self.functions_detailed.values() if f["is_method"]
            ),
            "total_async": sum(
                1 for f in self.functions_detailed.values() if f["is_async"]
            ),
            "documented_functions": sum(
                1 for f in self.functions_detailed.values() if f["has_docstring"]
            ),
    ...
    def generate_csv_report(self, prefix: str = "functions_analysis"):
        ...
        with open(funcs_csv, "w", newline="", encoding="utf-8") as f:
            ...
            for func in sorted_funcs:
                func_type = (
                    "Async Method"
                    if func["is_async"] and func["is_method"]
                    else (
                        "Method"
                        if func["is_method"]
                        else "Async Function" if func["is_async"] else "Function"
                    )
                )
        ...
        with open(summary_csv, "w", newline="", encoding="utf-8") as f:
            ...
            for category, count in sorted(
                ...
                category_info = self.function_analyzer.FUNCTION_CATEGORIES.get(
                    category, {}
                )
                ...
                examples = [
                    f["name"]
                    for f in self.functions_detailed.values()
        ...
        with open(calls_csv, "w", newline="", encoding="utf-8") as f:
            ...
            for func_name, calls in sorted(
                self.function_calls.items(), key=lambda x: len(x[1]), reverse=True
            )[:100]:
                unique_files = set(call["file"] for call in calls)
                writer.writerow([func_name, len(calls), len(unique_files)])
...
"""
        ...
        for category, count in sorted(
            ...
            percentage = (
                round((count / stats["total_functions"] * 100), 2)
                if stats["total_functions"] > 0
                else 0
            )
...
"""
            ...
            for func in sorted_funcs:
                ...
                if func["is_method"]:
                    func_type.append(
                        f'<span class="method">method of {func["class_name"]}</span>'
                    )
                if not func_type:
                    func_type.append("function")

                doc_status = (
                    '<span class="documented">✓</span>'
